...
func SetWaitingForLoanValue(rdb *redis.Client, chatID int64, waiting bool) error {
	ctx := context.Background()
	key := fmt.Sprintf("waiting_loan_value:%d", chatID)
...
func IsWaitingForLoanValue(rdb *redis.Client, chatID int64) bool {
	ctx := context.Background()
	key := fmt.Sprintf("waiting_loan_value:%d", chatID)
